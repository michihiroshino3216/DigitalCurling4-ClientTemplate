...
LOG_PATH = Path(__file__).parent.parent / 'dc4_team1_20260303_090906.jsonl'
...
def map_shooter(per_shot):
    # shooter team for each (end,total)
    # total=0 entry has next_shot=first shooter
    by_end = {}
...
def write_outputs(entries, outdir: Path):
    # latest snapshot per end and per (end,total)
    per_end = {}
    per_shot = {}
    for e in entries:
        per_end[e['end']] = e
    ...
    # per-shot state csv (both teams, each shot)
