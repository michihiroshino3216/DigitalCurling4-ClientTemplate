...
def choose_improved_strategy_shot(board, my_team, logger):
    ...
    if house_opp and len(house_my) < len(house_opp):
        ...
        kind = "freeze"
        tx, ty = freeze_target["x"], freeze_target["y"] - 0.25  # 少し手前
        reason = f"Freeze: aiming in front of opp stone at ({freeze_target['x']:.2f}, {freeze_target['y']:.2f})"
...
async def main():
    ...
    logger = logging.getLogger("ShotEngine_B_Freeze")
    ...
    try:
        ...
        logger.info(f"=== I AM {my_team} (Freeze Strategy) ===")
