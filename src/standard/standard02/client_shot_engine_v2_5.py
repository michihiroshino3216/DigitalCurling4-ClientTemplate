...
def choose_simple_strategy_shot(board, my_team):
    ...
    if dangerous and is_in_house(dangerous["x"], dangerous["y"]):
        kind = "takeout"
...
def convert_center_draw(tx, ty):
    """
    センタードロー用の単純な速度計算。
    目標点までの距離に比例して速度をスケーリング。
    """
    d = dist(0, 0, tx, ty)
    base_v = 2.35  # TEE まで届く程度の基準速度
    v = base_v * (d / TEE_Y)

    angle = 0.0              # ほぼ直進
    omega = math.pi / 2      # 標準的な回転
...
def convert_takeout(tx, ty):
    """
    相手ナンバーワンテイクアウト用のショット。
    まずは固定値ベースで、あとからログを見てチューニングしていく前提。
    """
    # ひとまず「やや強めのヒット」くらいのイメージ
    v = 2.8

    # x 方向のズレに応じてわずかに角度をつける（超ざっくり）
    # 原点(0,0)から見た目標位置の角度を使う
    angle = math.atan2(tx, ty)  # ty は正方向（ハウス側）
