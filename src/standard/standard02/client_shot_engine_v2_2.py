...
# ログから見える挙動に合わせた簡易カール係数（精度向上のため調整）
CURL_FACTOR = 0.05
...
def generate_candidates(board, my_team, turn_number):
    ...
    if turn_number <= 2:
        ...
        if board["dangerous"]:
            ...
            for offset in [-0.1, 0.0, 0.1]:  # オフセットを減らして精度向上
...
async def main():
    ...
    # あなた専用のショットエンジン用ロガー
    logger = logging.getLogger("ShotEngineV2")
    ...
    try:
        ...
        logger.info(f"=== I AM {my_team} (Balanced Attack-Defense AI) ===")
