...
class GridShotSelector:
    """グリッドから初速度を取り、omega による横ずれを予測して角度を補正するバージョン。

    目的: omega による滑走中の曲がりを推定し、出だし角度を補正して「レーン外れ」を減らす。
    簡易モデルのためパラメータは経験的な係数で与える。
    """
    ...
    def _predict_lateral_disp(self, omega: float, v: float, path_len: float) -> float:
        """簡易予測: 横ずれ量（m）を返す。

        - omega の符号により曲がる方向が決まる（omega>0 -> ccw -> 左へ曲がる）。
        - v が大きいほど曲がりは小さくなる（慣性）。
        - path_len は滑走長に相当し、grid にある source_distance を使える。
        """
        if v <= 0:
            return 0.0
        alpha = 0.03  # 経験的スケール（要調整）
    ...
    def get_shot_params(self, tx: float, ty: float) -> Tuple[float, float, float]:
        ...
        v = math.hypot(vx, vy)
        raw_angle = math.atan2(vx, vy)

        # source_distance があれば滑走長の代理として使う（無ければ 1.0）
        src_dist = entry.get("source_distance", 1.0)

        # 予測横ずれ（m）。正なら左方向へのずれ。
        pred_disp = self._predict_lateral_disp(omega, v, src_dist)

        # 角度補正: 目標までの直線距離を考慮して、予想される横ずれを打ち消す角度を算出
        # 補正角 = -atan(pred_disp / path_len) （path_len は前方距離の proxy として src_dist）
        # 補正を過大にしないために clamp
        if src_dist <= 0:
            corr_angle = 0.0
        else:
            corr_angle = -math.atan2(pred_disp, src_dist)
        max_corr = math.radians(20)
        corr_angle = max(-max_corr, min(max_corr, corr_angle))

        angle = raw_angle + corr_angle
...
# 以下は元の sample_client_no1_grid.py の main ループをベースにそのまま流用します。
# 実行方法やログは元ファイルと同様です。
...
async def main():
    ...
    logger = logging.getLogger("SampleClientNO1GridLaneSafe")
    ...
    file_handler = logging.FileHandler(logs_dir / "sample_client_no1_grid_lane_safe_debug.log", mode="a", encoding="utf-8")
    ...
    # メイン処理開始 — サーバから状態を受け取り、ショットを決定して送信します。
    # フロー:
    # - `choose_target` で目標座標とモードを選定
    # - `shot_to_target` で速度・角度・角速度を決定（'max' / 'spinner' 等をサポート）
    # - 決定したショットを `client.send_shot_info` で送信
    try:
        async for state_data in client.receive_state_data():
            ...
            if next_shot_team == my_team:
                ...
                if no1 is None:
                    logger.info(f"Shot {state_data.shot_number + 1}: target=({tx:.2f}, {ty:.2f}) (NO1 none)")
                else:
                    logger.info(f"Shot {state_data.shot_number + 1}: target=({tx:.2f}, {ty:.2f}) NO1=({no1['x']:.2f}, {no1['y']:.2f}) team={no1['team']}")
                logger.debug(
