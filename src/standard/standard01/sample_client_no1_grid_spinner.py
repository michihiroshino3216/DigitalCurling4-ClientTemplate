...
def shot_to_target(tx: float, ty: float, mode: str = None) -> Tuple[float, float, float, Dict]:
    ...
    if mode == 'spinner':
        # デフォルトでホグ間における回転数を 10-12 回転の中央値に設定（11 revs）
        revs = 11.0
        hog_dist = 6.4  # ホグライン間距離の代表値（必要に応じて調整）
        # 基本角度はグリッドから取得（レーン補正済み）
        _, angle, _, entry = _grid_selector.get_shot_params(tx, ty)
        # 打ち出し速度は遅めに設定（氷の状態に依るが 0.5-0.8 m/s が一般的）
        vel = 0.6
        # 回転方向はターゲットの左右で選ぶ（右側負、左側正）
        direction = 'ccw' if tx >= 0.0 else 'cw'
        sign = 1.0 if direction == 'ccw' else -1.0
        # omega 計算: revs = (omega/(2*pi)) * (hog_dist/vel) => omega = revs * 2*pi * vel / hog_dist
        omega = sign * (revs * 2.0 * math.pi * vel / hog_dist)
        return vel, angle, omega, entry
...
async def main():
    ...
    logger = logging.getLogger("SampleClientNO1GridSpinner")
    ...
    file_handler = logging.FileHandler(logs_dir / "sample_client_no1_grid_spinner_debug.log", mode="a", encoding="utf-8")
