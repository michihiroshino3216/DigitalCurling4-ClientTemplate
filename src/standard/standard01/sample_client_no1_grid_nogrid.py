...
# client_shot_engine_v2_6_freeze.py 由来の変換関数（トップレベル）
def convert_center_draw(tx: float, ty: float) -> Tuple[float, float, float]:
    v = 2.42
...
def convert_takeout(tx: float, ty: float) -> Tuple[float, float, float]:
...
def convert_guard(tx: float, ty: float) -> Tuple[float, float, float]:
    v = 2.33
    angle = math.radians(90)
    omega = math.pi / 2
    return v, angle, omega
...
def convert_freeze(tx: float, ty: float) -> Tuple[float, float, float]:
    v = 2.30
    angle = math.radians(90)
    omega = math.pi / 1.8
    return v, angle, omega
...
def get_shot_params(tx: float, ty: float) -> Tuple[float, float, float, Dict]:
    """grid を使わず、freeze エンジン由来のパラメータのみを返す。

    戻り値は (v, angle, omega, matched_entry) の形だが、matched_entry は常に None。
    """
    # TEE を明示的に狙う場合
    if math.isclose(tx, TEE_X, abs_tol=1e-6) and math.isclose(ty, TEE_Y, abs_tol=1e-6):
        v, angle, omega = convert_center_draw(tx, ty)
        return v, angle, omega, None
...
def shot_to_target(tx: float, ty: float, mode: str = None) -> Tuple[float, float, float, Dict]:
    """grid を使わないバージョン。matched_entry は常に None を返す。"""
    if mode == 'max':
        angle = math.atan2(tx, ty)
        return MAX_TRANSLATIONAL_VELOCITY, angle, 0.0, None
    return get_shot_params(tx, ty)
...
async def main():
    ...
    logger = logging.getLogger("SampleClientNO1GridNoGrid")
    ...
    file_handler = logging.FileHandler(logs_dir / "sample_client_no1_grid_nogrid_debug.log", mode="a", encoding="utf-8")
